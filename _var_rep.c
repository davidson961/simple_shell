...
/**
 * env_checker - checks if the variable is an environment variable
 * @h: head of the list
 * @in: input
 * @data: shell info
 * Return: Non.
...
void env_checker(r_var **h, char *in, data_shell *data)
{
	...
	_environ = data->environment;
	for (rw = 0; _environ[rw]; rw++)
	{
		for (i = 1, ch = 0; _environ[rw][ch]; ch++)
		{
			if (_environ[rw][ch] == '=')
			{
				lval = _strlen(_environ[rw] + ch + 1);
				add_var_end(h, i, _environ[rw] + ch + 1, lval);
				return;
			}

			if (in[i] == _environ[rw][ch])
				i++;
			else
				break;
		}
...
/**
 * replace_var - replaces string into variables
 * @input: input
 * @datash: shell info
